...
function MagnifyImage({ src, alt, lensSize = 160, zoom = 2, className = '', onClick }) {
  ...
  useEffect(() => {
    ...
    // Keep size in sync with viewport changes
    window.addEventListener('resize', updateSize);

    // Observe container resizes (layout shifts, fonts, etc.)
    let observer;
  ...
  // Ensure correct size after image loads and when src changes
  useEffect(() => {
  ...
  return (
    <div
      ...
      role="img"
    >
      <img ref={imgRef} src={src} alt={alt} className="magnifier-image" loading="lazy" />
      {isHovering && (
  ...
  return (
        <Badge
          ...
          className={`mb-5 font-semibold py-2 px-4 rounded-full text-base 
            ...
            border border-blue-400/50 badge-vibrant
            text-blue-100 shadow-md 
    ...
    <BackgroundWrapper>
      ...
      {/* {activeFeature && (
        ...
        >
          ...
          >
            <button
              className="absolute -top-3 -right-3 bg-card/90 text-card-foreground rounded-full w-9 h-9 flex items-center justify-center shadow-md border border-border/60"
              onClick={() => setActiveFeature(null)}
              aria-label="Close"
            >
              ✕
